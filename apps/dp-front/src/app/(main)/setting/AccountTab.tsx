...
export default function AccountTab() {
  ...
  return (
    <>
      ...
      >
        <div
          className="flex h-14 w-14 flex-none items-center justify-center rounded-[18px] text-[28px] text-white"
          style={{
      ...
      <Row
        ...
        onClick={notifyComingSoon}
      />
      <Row icon="🔐" title="비밀번호 변경" sub="소셜 로그인 연결" onClick={notifyComingSoon} />
      <Row icon="🪪" title="본인확인 정보" sub="PASS · 1년 캐시" onClick={notifyComingSoon} />
      <Row icon="🌐" title="언어" sub="한국어" onClick={notifyComingSoon} />
      <Row icon="🇰🇷" title="국가 / 지역" sub="대한민국" onClick={notifyComingSoon} last />
    </>
  );
}
